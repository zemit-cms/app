...
class ErrorsController extends AbstractController {
     ...
     */
    private function _restError($error = null) {
        ...
        $exceptions = array();
        if (empty($error)) {
            $error = \Phalcon\Text::uncamelize($this->dispatcher->getControllerName() . '_' . $this->dispatcher->getActionName() . '_' . $statusCode);
        }
        ...
        return array(
            'error' => true,
            'from' => __NAMESPACE__ . '\\' . ucfirst($this->dispatcher->getPreviousControllerName()) . '->' . $this->dispatcher->getPreviousActionName() . '()',
            'type' => $error,
            'status' => $status,
            'code' => $statusCode,
